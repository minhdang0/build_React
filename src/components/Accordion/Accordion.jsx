...
const Accordion = ({ defaultIndex = 0, children, onChange, collapseOthers }) => {
    const [currentIndex, setCurrentIndex] = useState(defaultIndex);
    const [highlightIndex, setHighlightIndex] = useState(defaultIndex);
    const accordions = Children.toArray(children);
    const [collapses, setCollapses] = useState(
        Array(accordions.length).fill(false).map((_, index) => index === defaultIndex)
    );
    ...
    const handleKeyDown = (e) => {
        ...
        if (['ArrowDown', 'ArrowRight'].includes(e.key)) {
            ...
            setCollapses((prev) => {
                ...
                if (collapseOthers) return prev.map((_, i) => i === highlightIndex ? !isOpen : false);
                else {
    ...
    return (
        <div className={styles.wrapper} tabIndex={0} ref={wrapperRef} onKeyDown={handleKeyDown}>
                            ...
                            <button key={index}
                                onClick={() => {
                                    ...
                                    setCollapses((prev) => {
                                        ...
                                        if (collapseOthers) return prev.map((_, i) => i === index ? !isOpen : false);
                                        else {
